...
APPDIR="$BUILDDIR/actilectrum.AppDir"
...
APPIMAGE="$DISTDIR/actilectrum-$VERSION-x86_64.AppImage"
...
cp -f "$PROJECT_ROOT/actilectrum/libsecp256k1.so.0" "$APPDIR/usr/lib/libsecp256k1.so.0" || fail "Could not copy libsecp to its destination"
...
(
    ...
    pushd "$CONTRIB"/deterministic-build/locale
    if ! which msgfmt > /dev/null 2>&1; then
        fail "Please install gettext"
    fi
    for i in ./locale/*; do
        dir="$PROJECT_ROOT/actilectrum/$i/LC_MESSAGES"
        mkdir -p $dir
...
cp "$PROJECT_ROOT/actilectrum.desktop" "$APPDIR/actilectrum.desktop"
cp "$PROJECT_ROOT/actilectrum/gui/icons/actilectrum.png" "$APPDIR/actilectrum.png"
