...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/actilectrum-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\actilectrum\actilectrum\gui\icons\actilectrum.ico"
...
Section
  ...
  File /r "dist\actilectrum\*.*"
  File "c:\actilectrum\actilectrum\gui\icons\actilectrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\actilectrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\actilectrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\actilectrum-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\actinium" "DefaultIcon" "$\"$INSTDIR\actilectrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\actinium\shell\open\command" "" "$\"$INSTDIR\actilectrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\actilectrum.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\actinium"
