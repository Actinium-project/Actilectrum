...
cp "$PROJECT_ROOT/actilectrum/libsecp256k1-0.dll" $WINEPREFIX/drive_c/tmp/ || fail "Could not copy libsecp to its destination"
...
(
    cd "$WINEPREFIX/drive_c/actilectrum"
    ...
    echo "const char *electrum_tag = \"tagged by Actilectrum@$ELECTRUM_COMMIT_HASH\";" >> ./bootloader/src/pyi_main.c
    pushd bootloader
