...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/actilectrum-macos
	cd /tmp/actilectrum-macos
...
rm -rf /tmp/actilectrum-macos/image > /dev/null 2>&1
mkdir /tmp/actilectrum-macos/image/
cp -r $1 /tmp/actilectrum-macos/image/
