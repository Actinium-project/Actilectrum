...
class SimpleConfig(Logger):
    ...
    def actilectrum_path(self):
        ...
        self.logger.info(f"actilectrum directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'acm':8, 'macm':5, 'uacm':2, 'bits':2, 'sat':0}
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "actilectrum.dat")
...
def read_user_config(path):
    """Parse and store the user config settings in actilectrum.conf into user_config[]."""
    if not path:
        return {}
