...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("actilectrum."):
...
electrum_logger = logging.getLogger("actilectrum")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("actilectrum."):
        name = name[13:]
...
def configure_logging(config):
    ...
    _logger.info(f"Electrum-LTC version: {ELECTRUM_VERSION} - https://actilectrum.org - https://github.com/Actinium-project/Actilectrum")
