...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/actinium/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/actinium/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/actinium?localization=false&tickers=false&community_data=false&developer_data=false&sparkline=false')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/actinium/market_chart?vs_currency=%s&days=max' % ccy)
